...
Context::Context(Context* parent,
  ...
  : parent(parent)
  , name(name)
  , isFunctionCtx(isFunctionCtx)
  , isCompositeCtx(false)
...
Context::Context(Context* parent, const std::string& name, bool isCompositeCtx)
  ...
  , name(name)
  , isFunctionCtx(false)
  , isCompositeCtx(isCompositeCtx)
  , returnType(PSC::DataType::NONE)
{
}

template<typename T, typename... Args>
void copyPtrVector(const std::vector<std::unique_ptr<T>>& source,
                   ...
                   Args&&... args)
{
  ...
  for (size_t i = 0; i < size; i++) {
    dest.emplace_back(
      std::make_unique<T>(*source[i], std::forward<Args>(args)...));
  }
}
...
PSC::DataType Context::getType(const Token& token, bool global)
{
  if (token.type == TokenType::DATA_TYPE) {
    if (token.value == "INTEGER")
      return PSC::DataType(PSC::DataType::INTEGER);
    else if (token.value == "REAL")
      return PSC::DataType(PSC::DataType::REAL);
    else if (token.value == "BOOLEAN")
      return PSC::DataType(PSC::DataType::BOOLEAN);
    else if (token.value == "CHAR")
      return PSC::DataType(PSC::DataType::CHAR);
    else if (token.value == "STRING")
      return PSC::DataType(PSC::DataType::STRING);
    else if (token.value == "DATE")
      return PSC::DataType(PSC::DataType::DATE);
    else
      std::abort();
  }
    ...
    if (compositeDefinition != nullptr)
      return PSC::DataType(PSC::DataType::COMPOSITE,
...
Enum* Context::getEnumElement(const std::string& value, bool global)
{
  ...
  if (global && parent != nullptr)
    return getGlobalContext()->getEnumElement(value);
  return nullptr;
}
...
{
  enums.emplace_back(
    std::make_unique<EnumTypeDefinition>(std::move(definition)));
}

void Context::createPointerDefinition(PointerTypeDefinition&& definition)
{
  pointers.emplace_back(
    std::make_unique<PointerTypeDefinition>(std::move(definition)));
}

void Context::createCompositeDefinition(CompositeTypeDefinition&& definition)
{
  composites.emplace_back(
    std::make_unique<CompositeTypeDefinition>(std::move(definition)));
}
                                                     ...
                                                     bool global)
{
  ...
  if (global && parent != nullptr)
    return getGlobalContext()->getEnumDefinition(name);
  return nullptr;
}

const PointerTypeDefinition* Context::getPointerDefinition(
  const std::string& name, bool global)
{
  ...
  if (global && parent != nullptr)
    return getGlobalContext()->getPointerDefinition(name);
  return nullptr;
}

const CompositeTypeDefinition* Context::getCompositeDefinition(
  const std::string& name, bool global)
{
  ...
  if (global && parent != nullptr)
    return getGlobalContext()->getCompositeDefinition(name);
  return nullptr;
}
